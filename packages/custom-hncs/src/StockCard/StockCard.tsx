...
function StockCard({
    ...
    return (
      <div
        className={`stock-change
          ${priceChange > 0 ? "stock-change-up" : priceChange == 0 ? "stock-change-normal" : "stock-change-down"}`}
      >
    ...
    return (
      <div className="stock-chart">
        <Sparklines data={prices.length > 0 ? prices : [0, 0, 0, 0, 0]}>
          <SparklinesLine color={priceChange > 0 ? "#6FCF97" : priceChange == 0 ? "#bab8c0" : "#EB5757"} />
        </Sparklines>
      </div>
    );
  };
  ...
  return (
    <div className={`${className} stock-card-Cont card--glassmorphism`}>
      {loading ? (
