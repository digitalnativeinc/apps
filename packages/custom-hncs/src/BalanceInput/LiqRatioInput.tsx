...
function LiqNumberFormat(props: any): React.ReactElement {
  ...
  return (
    <NumberFormat
      suffix={"%"}
      className={`${props.liqRatio < props.min ? "error-text" : "success-text"}`}
      decimalScale={2}
  ...
  return (
    <div
      ...
      onClick={e => {
        ...
        const degFromTan = (opp: number, adj: number) => (Math.atan(opp / adj) * 180) / Math.PI;
        let ang;
        if (x < 0) {
          // 1st quad
          if (y < 0) {
        ...
        <div style={{ width: "calc(90%)", position: "relative" }}>
          <CircularProgressbar
            ...
            styles={buildStyles({
              ...
              pathColor: `${fill < 37.5 ? themeContext.red : themeContext.green}`
            })}
            classes={{
          ...
          <div className="liq-ratio-cont">
            <LiqNumberFormat max={max} min={min} liqRatio={liqRatio} setLiqRatio={setLiqRatio} />
